...
## Admin bootstrap
No admin credential file is bundled. Set `ADMIN_USER_ID` and `ADMIN_PASSWORD` through the deployment environment. The provisioning script stores only an scrypt password hash in PostgreSQL.
...
Set `CORS_ORIGIN` to the comma-separated exact origins of the deployed USER and ADMIN frontends. Set `JWT_SECRET` to a strong secret. The frontend defaults to the Render core API origin but can be overridden with `window.TREDIN_CONFIG.apiBaseUrl`.

## Render Free-plan bootstrap
The root `render.yaml` intentionally does not use `preDeployCommand` for the Core API because Render documents pre-deploy commands as unavailable on Free web services. The Free-compatible start sequence runs the idempotent database initialization, instrument seed, admin provisioning, and API startup in order.
